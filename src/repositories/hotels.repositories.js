...
export function getHotelsByCityIdDB(cityId) {
  return db.query(
    `
      SELECT hotels.id, hotels.name, hotels.description, hotels.price, hotels.avatar, JSON_BUILD_OBJECT('id', cities.id, 'name', cities.name) AS city
      FROM hotels
      JOIN cities ON hotels."cityId" = cities.id
      WHERE cities.id = $1
      GROUP BY hotels.id, hotels.name, hotels.description, hotels.price, cities.id, cities.name;
    `,
    [cityId]
  );
}
...
export function getHotelsDetailsDB(hotelId) {
  return db.query(
    `
      SELECT hotels.id, hotels.name, hotels.description, hotels.price, hotels.avatar, JSON_BUILD_OBJECT('id', cities.id, 'name', cities.name) AS city, COALESCE(ARRAY_AGG(DISTINCT photos.url), ARRAY[]::text[]) AS photos, ARRAY_AGG(DISTINCT conveniences.obj) AS conveniences
      FROM hotels
      JOIN cities ON hotels."cityId" = cities.id
